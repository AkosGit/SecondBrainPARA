...
| `ResourceTemplate` | Finished artifact | **Location-aware.** Inside `PROJECTS/<X>/` or `ARCHIVE/<X>/` it produces a project deliverable — Parent → the project's `00000`, stamps `Project:`, inherits the project index's tags, offers a board card — i.e. the same result as `ProjectFileTemplate` → Resource. Anywhere else it moves the note to `RESOURCES/` and asks for area + topics. ⚠️ Its `addBoardCard()` is duplicated from `ProjectFileTemplate` — keep in sync. |
...
Also added, same day: `ProjectFileTemplate` now offers to append a link-only kanban card when the note is a `Type: Resource` inside a project (§4).

Two follow-up fixes from first real use of that card prompt:

1. **The prompt came after the board lookup**, so a project without a board returned early and the feature looked broken rather than looking like "no board here". The prompt now fires first, and a missing board raises an 8-second Notice naming the folder. Board detection also widened to `Type: Tasks` **or** `kanban-plugin: board`, falling back to a `*Tasks` filename match, since a board created moments earlier by `ProjectIndexTemplate` may not be in the metadata cache yet.
2. **Reaching for `ResourceTemplate` inside a project silently produced the wrong shape** — Parent pointing at the area hub, no `Project:` field, no board card — because only `ProjectFileTemplate` knew about projects. Naming being what it is, "I'm making a resource" leads to the template with *Resource* in its name. `ResourceTemplate` is now location-aware and produces the identical result to `ProjectFileTemplate` → Resource when run inside a project folder. Scoped to Resources because a Resource in a project is a deliverable that usually owes a task, while Ideas and Sources usually don't. The card carries no metadata — the board still owns status, and the card is a pointer to the note, not a duplicate of it.
